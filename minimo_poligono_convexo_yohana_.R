...
lat_ini <- "25°04'26.48''S" |> 
  parzer::parse_lat()

lat_ini

lon_ini <- "51°49'53.63''W" |> 
  parzer::parse_lon()

lon_ini

lat_final <- "25°04'25.93''S" |> 
  parzer::parse_lat()

lat_final

lon_final <- "51°49'53.71''W" |> 
  parzer::parse_lon()
...
### Criando o shapefile de trajeyo reto ----

trajeto_reto <- trajeto[c(1, 71), ] |> 
  sf::st_as_sf(coords = c("lon", "lat"),
               crs = 4674) |>
  dplyr::summarise(do_union = FALSE) |>
  sf::st_cast("LINESTRING")
...
ggplot() +
  geom_sf(data = trajeto_reto, linewidth = 1) +
  geom_sf(data = trajeto_convexo, fill = "blue", alpha = 0.3)

### Calculando o índice de tortuosidade ----

indice <- (trajeto_reto |> sf::st_length()) / tamanho
